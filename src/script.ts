...
const getScript = (isTelemetryEnabled: boolean, opencreditsApiUrl: string = 'https://ccc.api.opencredits.ai', opencreditsWebUrl: string = 'https://ccc.opencredits.ai', opencreditsPublishableKey: string = 'oc_pk_c43da4f9a9484ae484ad29bc97cc354f') => `<script>
		...
		function scrollToBottomIfNeeded(messagesDiv, shouldScroll = null) {
			...
			function addMessage(content, type = 'claude') {
				...
				if ((type === 'user' || type === 'error' || type === 'thinking') && headerDiv) {
					makeTranscriptRowCollapsible(messageDiv, headerDiv, contentDiv, shouldDefaultCollapseRow(type));
				}
			...
			function shouldDefaultCollapseRow(type) {
				return type !== 'claude';
			}

			function toggleTranscriptRow(rowId) {
				const row = document.getElementById(rowId);
				if (!row) return;
				const collapsed = row.classList.toggle('collapsed');
				const chevron = row.querySelector('.transcript-collapse-chevron');
				const toggleBtn = row.querySelector('.transcript-collapse-btn');
				if (chevron) {
			...
			function makeTranscriptRowCollapsible(messageDiv, headerDiv, bodyElement, collapsed) {
				const rowId = 'row_' + Math.random().toString(36).slice(2, 10);
				messageDiv.id = rowId;
				messageDiv.classList.add('transcript-collapsible');
				bodyElement.classList.add('transcript-collapsible-body');
				const toggleBtn = document.createElement('button');
				toggleBtn.className = 'transcript-collapse-btn';
				toggleBtn.type = 'button';
				toggleBtn.title = collapsed ? 'Expand' : 'Collapse';
				toggleBtn.setAttribute('onclick', 'toggleTranscriptRow("' + rowId + '")');
				toggleBtn.innerHTML = '<span class="transcript-collapse-chevron">' + (collapsed ? '▸' : '▾') + '</span>';
			...
			function ensureActiveStreamMessage(type = 'claude') {
				...
				if (type === 'claude' || type === 'thinking') {
					...
					const iconDiv = document.createElement('div');
					iconDiv.className = 'message-icon ' + (type === 'thinking' ? 'system' : type);

					const labelDiv = document.createElement('div');
					labelDiv.className = 'message-label';
					labelDiv.textContent = type === 'thinking' ? 'Thought' : 'Claude';
		...
		function addToolUseMessage(data) {
			...
			if (data.rawInput) {
				...
				if (data.toolName === 'TodoWrite' && data.rawInput.todos) {
					...
					for (const todo of data.rawInput.todos) {
						...
						todoHtml += String.fromCharCode(10) + status + ' ' + todo.content;
					}
			...
			if (bodyElement) {
				makeTranscriptRowCollapsible(messageDiv, headerDiv, bodyElement, true);
			}
			messagesDiv.appendChild(messageDiv);
			moveProcessingIndicatorToLast();
			scrollToBottomIfNeeded(messagesDiv, shouldScroll);
		...
		function addToolResultMessage(data) {
			...
			messageDiv.appendChild(contentDiv);
			makeTranscriptRowCollapsible(messageDiv, headerDiv, contentDiv, true);
