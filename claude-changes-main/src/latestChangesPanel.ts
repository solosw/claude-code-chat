...
export class LatestChangesViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = "claudeChangesLatest";
  ...
  private async _update(webview: vscode.Webview): Promise<void> {
    if (!this._workspacePath) {
      webview.html = this._getEmptyHtml("No workspace folder open");
      return;
    }

    const sessions = await findSessionsForWorkspace(
      this._workspacePath,
      this._log
    );
...
<body><div class="empty">${message}</div></body></html>`;
  ...
  private _getHtml(sessions: SessionInfo[]): string {
    ...
    const sessionTitle = this._escapeHtml(
...
<body>
  ...
  private _getLatestFiles(session: SessionInfo): FileBackup[] {
  ...
  private _hasActualDiff(session: SessionInfo, snapshot: Snapshot, file: FileBackup): boolean {
    ...
    if (file.backupFileName === null) {
      ...
      if (!fs.existsSync(file.absolutePath)) return false;
      try {
  ...
  private _renderFileItem(session: SessionInfo, f: FileBackup): string {
    ...
    const lastSnap = session.snapshots[session.snapshots.length - 1];
    const nextBackup = findNextBackup(session, lastSnap.messageId, f.filePath);
