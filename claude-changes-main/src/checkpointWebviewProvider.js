...
exports.CheckpointWebviewProvider = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const checkpointService_1 = require("./checkpointService");
class CheckpointWebviewProvider {
    workspacePath;
    log;
    static viewType = "claudeChanges";
    _view;
    ...
    markAllReverted(sessionId, absolutePaths) {
        ...
        webviewView.webview.onDidReceiveMessage((message) => {
            switch (message.command) {
                ...
                case "restoreFile":
                    ...
                    break;
                case "revertAll":
                    vscode.commands.executeCommand("claudeChanges.revertAllData", message.sessionId);
                    break;
                case "deleteFile":
    ...
    async _updateContent(webview) {
        if (!this.workspacePath) {
            webview.html = this._getEmptyHtml("No workspace folder open");
            return;
        }
        const sessions = await (0, checkpointService_1.findSessionsForWorkspace)(this.workspacePath, this.log);
...
<body>
  ...
  <script>
    ...
    _renderSession(session, index) {
        ...
        const title = this._escapeHtml(session.firstUserMessage || session.slug || session.sessionId.slice(0, 8));
        const fullTitle = session.firstUserMessage || session.slug || session.sessionId;
        ...
        const cumulativeFiles = (0, checkpointService_1.getCumulativeChanges)(session).filter((f) => this._hasCumulativeDiff(session, f));
        ...
        const timelineCountLabel = timelineCheckpointCount === totalCheckpointCount
            ? `${timelineCheckpointCount} checkpoint${timelineCheckpointCount !== 1 ? "s" : ""}`
            : `${timelineCheckpointCount} shown / ${totalCheckpointCount} total`;
        const hiddenCheckpointTooltip = hiddenCheckpointCount > 0
            ? ` title="${this._escapeAttr(`${hiddenCheckpointCount} checkpoint${hiddenCheckpointCount !== 1 ? "s are" : " is"} hidden because they have no actual diff`)}"`
            : "";
        const checkpointsHtml = [...timelineCheckpoints]
            .reverse()
            .map((entry) => this._renderCheckpoint(session, entry.snap, entry.originalIndex, entry.files))
            .join("");
        return `
    ...
    _renderCheckpoint(session, snapshot, index, files) {
    ...
    _hasCumulativeDiff(session, file) {
    ...
    _hasTimelineDiff(session, snapshot, file) {
    ...
    _renderFileItem(session, f, mode, snapshot) {
        const fileName = path.basename(f.absolutePath);
        const nextBackup = mode === "checkpoint" && snapshot
            ? (0, checkpointService_1.findNextBackup)(session, snapshot.messageId, f.filePath)
            : null;
    ...
    _revertedKey(sessionId, absolutePath) {
        return `${sessionId}::${absolutePath}`;
    }
}
exports.CheckpointWebviewProvider = CheckpointWebviewProvider;
//# sourceMappingURL=checkpointWebviewProvider.js.map